...
**🚀 SHIPPED — v1.0.0 is LIVE in production:** https://velmora-phi.vercel.app (Vercel, team `richter83-7779's projects`). `main` + `phase-1-foundation` + tag `v1.0.0` all at the deployed commit; GitHub Release published. Everything below is done: core game (Phases 0–12), riso redesign, the **Dark Mirrors expansion** (5 paths + 18 finale endings), the **AI replayability stack** (on-device AI Director + Story Weaver, opt-in BYOK Live Storyteller), and **consumer-ready launch polish** (security headers/CSP, absolute SEO/social metadata + 1200×630 social card + JSON-LD + sitemap, hardened PWA manifest + install screenshots, served Privacy/Terms + in-app links). **Live-verified:** HTTP 200, A-grade security headers (CSP allows the BYOK api.anthropic.com call), all assets 200, zero console/CSP violations on load, axe-clean, offline intact. **Only manual step left:** confirm the Lighthouse score via PageSpeed Insights / DevTools on the live URL (headless Lighthouse CLI hits the documented NO_FCP env flake; the app paints fine — the deterministic `npm run size` is the hard perf gate). Decisions locked: monetization = free base + paid expansion (entitlement scaffold in place); distribution = web.

**▶ NEXT SESSION (2026-06-25) — FULL PRODUCTION REVAMP (user direction):** the current look is "too basic" — make it **cinematic and character-driven**. A major visual/experience overhaul: atmosphere, motion, art direction, and especially **CHARACTER appeal** (the player / rival / advisors should feel like real characters, not parametric avatars). Durable layers (engine, content, persistence, a11y structure, the AI Director / Story Weaver / Live Storyteller, the 5 paths) carry over untouched — this is a presentation/feel revamp on top. Custom domain deferred until then (account has only `sitescout.it.com` / `mechonwheels.info`; a dedicated domain like `velmora.app` is the better launch home — decide during the revamp). Live URL stays `https://velmora-phi.vercel.app`.
...
## Live Storyteller — opt-in BYOK online LLM (Layer 2) — complete

Layer 2 of the AI-replayability plan and the one genuine fork (network + a secret + open generation), built to the hardened bar the adversarial review demanded. **Default OFF; the default game is untouched.**

- **Quarantined under `src/live/**`** (never imported by `src/engine`or`src/content` — enforced by a CI guard test), lazy-loaded only when the user opts in, so the offline/deterministic core and the entry bundle never carry it.
- **The contract (`contract.ts`):** `LiveEventSchema` is a strict SUBSET of the production event schema — no engine-semantic fields (no req/roll/then/sub/arcSet/npcFx/ending; an LLM must not author control flow), `body` always a plain string, fx restricted to the six stat keys with a hard ±25 clamp and `.strict()` rejecting unknown keys, ≤3 choices. The model is forced to emit exactly one `emit_event` tool call (structured JSON, never prose).
- **The hard safety gate (`safety.ts`):** EVERY generated string passes, before display: LiveEventSchema → coerce to a `live_` GameEvent → the production Zod `EventSchema` (defense in depth) → the **STRICT** denylist (CORE + the broad open-generation net: real nations/parties/institutions/figures/faiths) over the candidate event ONLY. Any failure → null → on-device fallback. The model is the suggestion; this gate, not the prompt, is the guarantee.
- **Client + prompt (`client.ts`/`prompt.ts`):** direct browser→Anthropic fetch with the user's own key, `anthropic-version` + `anthropic-dangerous-direct-browser-access`, a 7s `AbortController`, forced tool. The system prompt restates the fiction/content rules (defense in depth) and the user message carries a compact non-identifying state vector. Any non-ok/timeout/offline/malformed → null.
- **Orchestration (`index.ts`):** `maybeLiveEvent` gates on flag-ON + key present + `navigator.onLine` + a per-run budget (12 calls) + a seeded coin-flip, then runs client→safety. The BYOK key lives in its OWN `localStorage` entry (`velmora_live_key`), read only here, never on any save/export/error-report payload.
- **Wiring (`main.js`):** `nextEvent` stays a synchronous on-device draw when the flag is OFF (default → byte-identical). When ON, it shows a non-interactive "Composing…" placeholder, lazy-loads the live chunk, tries one validated event, and renders on-device on ANY miss. Validated live events register into the pool + `S.liveCache` (resume rehydration, same contract as Loom). Settings adds a **Live Storyteller** toggle with an explicit disclosure (state leaves the device, the key is local, calls cost you money), a password key field, and a model picker defaulting to the cheap model.
- **Tests:** an **adversarial fixture battery** (`live-safety.test.ts`) — accepts clean output; rejects core + STRICT denylist hits (ideology/religion/figure/symbol/nation/party/institution/living-figure), out-of-range fx, unknown fx keys, engine-semantic fields, bad choice counts, junk. An **architecture guard** (`live-guard.test.ts`) asserts engine/content never import `src/live` AND no real `sk-ant-…` key literal ships. An **offline/degradation E2E** (`live.spec.ts`): flag-ON-but-endpoint-failing and flag-ON-without-key both play to an ending with zero console errors (identical to flag-OFF).
- **Gate (all green):** typecheck · lint · content:validate (10) · **223 unit** · build · size (entry within 70 kB — live is lazy; total JS 152.5 kB ≪ 300) · **40 E2E**. The seeded sweep never enables this path (proven), so determinism is wholly intact.
- **Honest limits (per the review):** the safety gate is a curated denylist + strict schema, not a learned classifier — a backstop, broad, and fail-closed (reject → fallback), but not infallible; `anthropic-dangerous-direct-browser-access` is a documented browser-direct posture the feature degrades gracefully around; and replay uplift here is metered by the player's own budget. The on-device Director + Loom remain the guaranteed, shareable-by-seed default.
