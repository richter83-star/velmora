...
- **Remaining Phase 5** (not started): skippable tutorial, settings panel (reduced-motion/contrast toggles, persisted via the localStorage+memory fallback), main menu, codex/almanac (reference for the factions/advisors/traits/endings systems), run-summary, responsive QA at the standard breakpoints, and an axe sweep on key screens.
